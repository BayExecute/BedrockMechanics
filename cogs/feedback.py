...
class Feedback(commands.Cog):
	...
	@slash.command(name="feedback", description="Use this command to communicate with the developer!", options = [Option("message", "Whatever you want to say!", Type.STRING, True),Option("feedback_type", "Type of your feedback!", Type.STRING, choices=[OptionChoice("Feedback", "Feedback"),OptionChoice("Bug Report", "Bug Report"),OptionChoice("Feature Advice", "Feature Advice")])])
	async def feedback(self, inter, message, feedback_type: str = "Feedback"):
		...
		feedback_embed = discord.Embed(title="Success", description=f"**Your {feedback_type} delivered successfully!**", color = discord.Color.from_rgb(63, 231, 255), timestamp = datetime.datetime.utcnow()) 
